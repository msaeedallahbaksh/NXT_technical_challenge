-- Initialize database with sample data for the AI Product Discovery Assistant

-- Create products table if not exists (handled by SQLModel, but this ensures data)
...
-- Sample search context data would be inserted during application usage
-- This is just to ensure the database has some test data to work with